...
async def _(event):
    ...
    msgid = msg.id
    await tbot.edit_message(chatid, msgid, "Click on the below button to read the torrents 👇", buttons=[[Button.inline('▶️', data=f'torrent-{sender}|{search}|{index}|{chatid}|{msgid}')], [Button.inline('❌', data=f'torrentstop-{sender}|{chatid}|{msgid}')]])
